...
The order matters: anything that sizes itself to the ground reads what the DEM publishes.
    ...
    python3 render_pda.py                             # -> FS25_Granja_bonita/map/overview.{png,dds}, satellite/
...
- **A platform's feather is measured against the land, not the water.**
  ...
  it as well.
- **PDA overview scale and alignment for 16x maps**: The Giants Engine 16x map PDA viewport
  expects the 8192 m playable area to be rendered into the central 2048x2048 px box of a
  4096x4096 px canvas (with a 1024 px non-playable margin on each side: left, right, top, bottom).
  Mapping playable metres $(x, y) \in [0, 8192]$ to canvas pixels:
  $px = 1024.0 + (x / 8192.0) \times 2048.0$
  $py = 1024.0 + (y / 8192.0) \times 2048.0$
  Rendering the playable area across the full 4096 px canvas or calculating margins incorrectly
  causes severe in-game PDA misalignment between the player coordinates / icon and the PDA map.
- **Draw PDA from native vector polygons and respect visual layer order**: Using the exact
  polygons from `info_layers/textures.json` (115 fields, 194 roads, 113 farmyards/town blocks,
  12 buildings) avoids line-width approximation errors and ensures perfect parity with
  `densityMap` and `infoLayer_farmlands.png`. Layer order is load-bearing: Background ->
  Fields -> Woods -> Water (River & Lake) -> Island (drawn over water) -> Farmyards/Towns ->
  Roads -> Bridges. Drawing water after islands erases the island from the map.
- **Overview texture formats for FS25**: The in-game map requires `map/overview.png` and
  `map/overview.dds` (BC1_UNORM with 13 mipmaps, built with `texconv.exe -f BC1_UNORM -m 13 -y`),
  as well as `satellite/overview.png`.
